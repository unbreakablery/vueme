...
function get_all_msg($con,$config) {
	...
	while ($chat = mysqli_fetch_array($query)) {
        ...
        $to_id = $from_userdata[$TFuserid];
        $picname = $from_userdata[$TFPicname];
        $status = $from_userdata[$TFOnline];
        ...
        $picname2 = $to_userdata[$TFPicname];

        $picname2 = ($picname2 == "")? "avatar_default.png" : $picname2;
