...
const actions = {
    getItems({ commit, rootState }) {
        ...
        commit(loading(true));
        return axios.get(rootState.util.entrypoint + '/api/v1/search/helpfuls?page=' + state.page + '&per_page=' + state.per_page)
            .then(response => {
    ...
    getAll({ commit, rootState }) {
        commit(loading(true));
        return axios.get(rootState.util.entrypoint + '/api/v1/search/helpfuls')
            .then(response => {
                commit(loading(false));
    ...
    saveHelpfuls({ commit, rootState }, param) {

        commit(loading(true));
        return axios.post(rootState.util.entrypoint + '/api/v1/user/save/helpfuls?id='+param.id + '&type=' +param.type)
            .then(response => {
                commit(loading(false));
