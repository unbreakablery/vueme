...
const actions = {
    getItems({commit, rootState}, param) {
        ...
        let endpoint = rootState.util.entrypoint + '/api/admin/review' + '?' + objectToGetParameters(param);
    ...
    update({ commit, rootState }, values) {
        ...
        values.per_page = state.per_page;
        return axios.put(rootState.util.entrypoint + '/api/admin/review?id=' + values.id, values).then((response) => {
            commit(success(response.data));
            return true
        }).catch(e => {
            commit(loading(false));
