...
const actions = {
    getSectionItems({ commit, rootState }, params) {
        commit(loading(true));
        return axios.get(rootState.util.entrypoint + '/api/v1/search/models?cat=' + params.cat + '&filters=' + JSON.stringify(params.filters))
            .then(response => {
    ...
    getCatItems({ commit, rootState }, params) {
        commit(loading(true));
        return axios.get(rootState.util.entrypoint + '/api/v1/search/models?cat=' + params.cat)
            .then(response => {
                commit(loading(false));
                commit(success(response.data.data));
                return true
    ...
    getItems({ commit, rootState }, params) {
        commit(loading(true));
        return axios.get(rootState.util.entrypoint + '/api/v1/search/models?page=' + state.page + '&per_page=' + state.per_page + '&cat=' + params.cat + '&filters=' + JSON.stringify(params.filters))
            .then(response => {
                commit(loading(false));
                commit(success(response.data.data));
                return true
    ...
    getAll({ commit, rootState }) {
        commit(loading(true));
        return axios.get(rootState.util.entrypoint + '/api/v1/search/models')
            .then(response => {
                commit(loading(false));
