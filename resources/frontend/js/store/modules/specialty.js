...
const actions = {
    getItems({ commit, rootState }) {
        ...
        commit(loading(true));
        return axios.get(rootState.util.entrypoint + '/api/v1/search/specialties?page=' + state.page + '&per_page=' + state.per_page)
            .then(response => {
    ...
    getAll({ commit, rootState }) {
        commit(loading(true));
        return axios.get(rootState.util.entrypoint + '/api/v1/search/specialties')
            .then(response => {
                commit(loading(false));
    ...
    getItem({ commit, rootState }, slug) {
        ...
        commit(loading(true));
        return axios.get(rootState.util.entrypoint + '/api/v1/search/specialties/' + slug)
            .then(response => {
                commit(loading(false));
