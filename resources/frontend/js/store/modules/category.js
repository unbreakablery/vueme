...
const actions = {
    getItems({ commit, rootState }) {
        ...
        commit(loading(true));
        return axios.get(rootState.util.entrypoint + '/api/v1/search/categories?page=' + state.page + '&per_page=' + state.per_page)
            .then(response => {
    ...
    getAll({ commit, rootState }) {
        commit(loading(true));
        return axios.get(rootState.util.entrypoint + '/api/v1/search/categories')
            .then(response => {
                commit(loading(false));
    ...
    getItem({ commit, rootState }, slug) {
        ...
        commit(loading(true));
        return axios.get(rootState.util.entrypoint + '/api/v1/search/categories/' + slug)
            .then(response => {
                commit(loading(false));
